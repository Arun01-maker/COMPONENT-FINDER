...
def search():
    ...
    def generate():
        ...
        # Send list of site names to frontend to initialize the availability checklist UI
        site_names = [s["name"] for s in sites]
        yield f"data: {json.dumps({'type': 'init', 'sites': site_names})}\n\n"
        
        for site in sites:
            # Send 'searching' status
            yield f"data: {json.dumps({'type': 'status', 'site': site['name'], 'state': 'searching'})}\n\n"
            
            result = scrape_site(site, query)
            count = len(result.get("products", []))
            
            # Send 'done' status with availability detail
            yield f"data: {json.dumps({'type': 'status', 'site': site['name'], 'state': 'done', 'count': count})}\n\n"
